...
func main() {
	...
	clientConn, _, err := websocket.DefaultDialer.Dial("ws://127.0.0.1:3000/theater", nil)
	if err != nil {
		log.Fatal(err)
	}

	buffer, err := protocol.NewMsgProtobuf(proto.EMSG_LOGON, &proto.TheaterLogOnEvent{
		Token: []byte(accessToken),
		Room: []byte("5e87caef1b6bab962c89fc9e"),
	})
	if err != nil {
		log.Fatal(err)
	}
